...
function showResult() {
  ...
  if (btnShowResult) {
    btnShowResult.addEventListener("click", () => {
      ...
      const rectanglePresent = document.getElementById("a-2") !== null;
      const circlePresent =
        document.getElementById("a-2") === null &&
        document.getElementById("a-3") === null;
      const trianglePresent = document.getElementById("a-3") !== null;
...
function getAreaAndPerimeterOfATriangle(a, b, c) {
  // let area = Number(((a + b + c) / 2).toFixed(1));
  let s = (a + b + c) / 2;
